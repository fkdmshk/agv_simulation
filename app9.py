...
order = [
    st.sidebar.selectbox(f"巡回順序 {i+1}:", machine_list, index=i, key=f"order_{i}")
    for i in range(4)
]

# 各工程の滞留時間
st.sidebar.subheader("⏳ 各工程の滞留時間設定（秒）")
dwell_times = {machine: st.sidebar.number_input(f"{machine} 滞留時間:", min_value=1, max_value=10, value=3, step=1, key=f"dwell_{machine}") for machine in machines.keys()}
...
def main():
    ...
    while step < 30:  # 🚀 **30秒で工場シミュレーションを完了**
        for agv_x, agv_y in agv_path:
            ...
            # 画面更新
            factory_plot.plotly_chart(draw_factory_layout(agv_x, agv_y), use_container_width=True, key=f"factory_plot_{step}")
